    ...
    <div class="space-y-5 animate-fade-in">
      ...
      @if (mostrarForm) {
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div class="h-1 rounded-full bg-gradient-to-r from-teal-500 to-emerald-500"></div>
          <div class="p-6">
            <div class="flex items-center justify-between mb-4">
              <h3 class="text-lg font-semibold text-gray-900">{{ editando ? 'Editar' : 'Nuevo' }} Arbitro</h3>
              @if (!editando) {
                <button mat-stroked-button (click)="abrirScannerDni()" type="button" class="!rounded-lg !text-teal-600 !border-teal-600">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 stagger-children">
          @for (a of arbitrosFiltrados; track a.id) {
            <div class="bg-white rounded-xl border border-gray-200 hover:shadow-xl hover:-translate-y-1 hover:border-teal-300 transition-all duration-200 overflow-hidden">
              <div class="h-20 relative bg-gradient-to-br from-teal-500 to-emerald-600">
              ...
              <div class="relative px-4 pb-4">
                <div class="relative -mt-10 inline-block">
                  @if (a.foto_url) {
                    <img [src]="resolveUrl(a.foto_url)" class="w-20 h-20 rounded-full object-cover border-4 border-white shadow-lg" alt="Foto">
                  } @else {
                    <div class="w-20 h-20 rounded-full flex items-center justify-center text-xl font-bold border-4 border-white shadow-lg text-white bg-gradient-to-br from-teal-500 to-emerald-600">
            ...
            <div class="overflow-x-auto">
              <table class="w-full">
                <thead class="bg-gray-50 border-b border-gray-200">
                  <tr>
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wide">Arbitro</th>
                    ...
                    <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                      <td class="px-4 py-3">
                        <div class="flex items-center gap-3">
                          @if (a.foto_url) {
                            <img [src]="resolveUrl(a.foto_url)" class="w-9 h-9 rounded-full object-cover shrink-0" alt="Foto">
                          } @else {
                            <div class="w-9 h-9 rounded-full flex items-center justify-center text-[11px] font-bold shrink-0 text-white bg-gradient-to-br from-teal-500 to-emerald-600">
                              {{ (a.nombre?.charAt(0) || '') + (a.apellido?.charAt(0) || '') }}
                            </div>
                          }
                          <div class="min-w-0">
                            <p class="font-medium text-gray-900 text-sm truncate">{{ a.apellido }}, {{ a.nombre }}</p>
                          </div>
                        </div>
                      </td>
                      <td class="px-4 py-3 text-sm text-gray-600 hidden sm:table-cell">{{ a.dni }}</td>
                      ...
                      <td class="px-4 py-3 text-right">
                        <button
                          ...
                          [matMenuTriggerFor]="menuArb">
                          <mat-icon class="!text-lg">more_vert</mat-icon>
                        </button>
                        <mat-menu #menuArb="matMenu">
                          <button mat-menu-item (click)="editar(a)"><mat-icon>edit</mat-icon> Editar</button>
                          @if (auth.isAdmin()) {
                            <button mat-menu-item (click)="eliminar(a)"><mat-icon>delete</mat-icon> Eliminar</button>
                          }
                        </mat-menu>
                      </td>
                    </tr>
