...
@Component({
  ...
  template: `
    <div class="min-h-screen flex items-center justify-center bg-slate-950 p-4">
      <mat-card class="w-full max-w-md !bg-slate-900 !border !border-slate-700 text-center p-8">
        @if (loading) {
          <mat-spinner class="mx-auto mb-4"></mat-spinner>
          <p class="text-slate-400">Verificando email...</p>
        } @else if (success) {
          <mat-icon class="!text-6xl text-green-400 mb-4">check_circle</mat-icon>
          <h2 class="text-xl font-bold text-slate-200 mb-2">Email verificado</h2>
          <p class="text-slate-400 mb-6">Ya podes iniciar sesion.</p>
          ...
          <h2 class="text-xl font-bold text-slate-200 mb-2">Error</h2>
