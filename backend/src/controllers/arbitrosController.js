...
/**
 * arbitrosController — Vista filtrada de personas con rol 'arbitro'.
  ...
  try {
    ...
    const personas = await Persona.findAll({
      ...
      include: [{
        ...
        where: rolWhere,
        include: [
          { model: Rol, as: 'rol', where: { codigo: 'arbitro' }, attributes: ['id', 'codigo', 'nombre', 'color'] },
...
export const crear = async (req, res) => {
  try {
    ...
    if (!rolArbitro) return res.status(500).json({ success: false, message: 'Rol "arbitro" no existe en el catalogo' });
    ...
    const dup = await PersonaRol.findOne({
      where: { persona_id: persona.id, rol_id: rolArbitro.id, torneo_id, activo: true },
    });
    if (dup) {
      return res.status(400).json({
        success: false,
        message: `${persona.apellido}, ${persona.nombre} ya esta registrado como arbitro en este torneo`,
      });
    }

    const asignacion = await PersonaRol.create({
      persona_id: persona.id,
      rol_id: rolArbitro.id,
      torneo_id,
      activo: true,
    });
...
export const actualizar = async (req, res) => {
  try {
    ...
    if (!ra) return res.status(404).json({ success: false, message: 'Arbitro no encontrado' });
